...
// Notify Message Formatter
function lineNotifyMessageFormatter(lineData, completionsData) {
  return `\nName: ${lineData['user_name']} \
    \nTokens: ${completionsData['total_tokens']} \
    \n-------------------- \
    \n${lineData['message_text']} \
    \n-------------------- \
    \n${completionsData['output_message'].trim()}`;
}

// Send Line Notify
function lineSendNotify(content){
  UrlFetchApp.fetch('https://notify-api.line.me/api/notify', {
    'headers': {
      'Authorization': `Bearer ${LINE_NOTIFICATION_TOKEN}`
    },
    'method': 'post',
    'payload': {
      'message': content
...
// Get Audio File
function lineGetAudioFile(messageId) {
  return UrlFetchApp.fetch(`https://api-data.line.me/v2/bot/message/${messageId}/content`, {
    "method": "GET",
    "headers": {
      "Authorization": `Bearer ${LINE_ACCESS_TOKEN}`,
    },
